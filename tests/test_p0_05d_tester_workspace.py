...
def test_source_has_exact_sequential_45_cases():
    cases = sync.parse_cases(SOURCE)
    assert [row["ID"] for row in cases] == [f"WA-{number:02d}" for number in range(1, 46)]
...
def test_manual_visible_qr_cases_keep_render_and_atomic_security_separate():
    ...
    assert cases["WA-37"]["Case"] == "Progressive QR: chọn 1 hiện 1"
    assert "đúng 1 signed QR/instruction" in cases["WA-37"]["PASS bắt buộc"]
...
def test_manual_progressive_disclosure_case_preserves_local_confirm_and_money_boundaries():
    case = {row["ID"]: row for row in sync.parse_cases(SOURCE)}["WA-40"]
    assert case["SPEC_ID"] == "CUST-WEB-MANUAL-TOPUP-PROGRESSIVE-DISCLOSURE-002"
    assert case["Mức"] == "🔴 chặn-bán-hàng"
    assert case["Môi trường"] == "local-render + local-temp-only + live-read-only-after-deploy"
    assert "/wallet/topup" in case["Route / role / viewport"]
    assert "VI/EN/ZH" in case["Route / role / viewport"]
    assert "QR/destination/code/history-transfer/reference/final-submit/QR-request đều `0`" in case["PASS bắt buộc"]
    assert "`5/5` enabled VND method" in case["PASS bắt buộc"]
    assert "logout/account-switch purge toàn draft" in case["PASS bắt buộc"]
    assert "manual POST `0`" in case["PASS bắt buộc"]
    assert "temp final submit tạo `1 pending_admin_review`" in case["PASS bắt buộc"]
    assert "decision/Xu/ledger/provider `0`" in case["PASS bắt buộc"]
    assert "form click từng chặn native select" in case["Canh lỗi cũ"]


def test_a09_cases_keep_vertical_data_truth_and_auth_accessibility_separate():
    cases = {row["ID"]: row for row in sync.parse_cases(SOURCE)}
    assert {cases[case_id]["SPEC_ID"] for case_id in ("WA-41", "WA-42", "WA-43")} == {
        "A09-AUTH-ADMIN-SHELL-VERTICAL-RESPONSIVE-LOCALE"
    }
    assert {cases[case_id]["Mức"] for case_id in ("WA-41", "WA-42")} == {"🔴 chặn-bán-hàng"}
    assert cases["WA-43"]["Mức"] == "🟠 nặng"
    assert "sidebar `256px`" in cases["WA-41"]["PASS bắt buộc"]
    assert "metric/chart `0`, empty state `1`" in cases["WA-42"]["PASS bắt buộc"]
    assert "logo contain ratio `1.00`" in cases["WA-43"]["PASS bắt buộc"]
    assert all("live-after-deploy" in cases[case_id]["Môi trường"] for case_id in ("WA-41", "WA-42", "WA-43"))
    assert all(cases[case_id]["Canh lỗi cũ"] for case_id in ("WA-41", "WA-42", "WA-43"))
    assert cases["WA-44"]["SPEC_ID"] == "A09-OPERATIONS-READINESS-TRUTH"
    assert cases["WA-45"]["SPEC_ID"] == "A09-ADMIN-RELIABILITY-UX-LOCALE"
    assert "enabled `20/20` và disabled `8/8`" in cases["WA-45"]["PASS bắt buộc"]
...
def test_guide_is_substantive_safe_and_separates_dry_run_from_write():
    ...
    for marker in ["WA-01..WA-45", "BASE", "HEAD", "runtime SHA", "local", "CI", "deployed", "live", "CSRF", "idempotency", "redaction", "PROVIDER_CALLS=0", "WALLET_MUTATIONS=0", "LIVE_MONEY_FLOW=NOT_TESTED", "#412", "--so=3", "--that", "TOAN AAS Web App · Tester P0"]:
...
def test_readiness_json_has_explicit_truth_and_file_metadata():
    ...
    assert data["push_gate"] == "A09_ADMIN_RELIABILITY_PR_PENDING"
