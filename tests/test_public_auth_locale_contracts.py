...
def test_public_auth_locale_copy_is_complete_without_new_browser_authority() -> None:
    ...
    for required in (
        ...
        'accessText("context.recoveryTitle", "Khôi phục quyền truy cập vào Workspace của bạn.")',
        'accessText("notice.registrationHandoffTitle", "Tiếp tục bằng đăng nhập")',
        'accessText("notice.registrationHandoffBody", "Nếu email vừa gửi chưa có tài khoản, hồ sơ đã được tạo. Đăng nhập để khởi tạo signed session và dùng Workspace Web; Telegram có thể liên kết sau nếu cần đồng bộ Bot.")',
