...
def test_mobile_workspace_dock_is_signed_session_only_and_navigation_only() -> None:
    ...
    # Labels follow the customer PWA language while the first item still
...
def test_job_asset_and_ticket_views_only_filter_redacted_canonical_metadata() -> None:
    ...
    assert "function assetRecordIdentity(item)" in PORTAL
    assert 'assetId.startsWith("wna:v1:")' in PORTAL
    assert 'data-delivery="vault">${deliveryCenterText("status.delivery.vault", "Tệp riêng Web · không phải output")}' in PORTAL
    assert 'data-portal-action="filter-assets"' not in PORTAL  # attributes are generated by filterBar
    assert 'filterBar(localizedDeliveryFilters("assets", ASSET_FILTERS), selected, "filter-assets"' in PORTAL
...
def test_telegram_onboarding_can_start_from_a_fresh_signed_web_account() -> None:
    ...
    assert 'renderEmpty(onboardingText("emptyTitle", "Chưa có mã liên kết")' in onboarding
...
def test_admin_writes_are_explicitly_flag_gated_confirmed_and_idempotent() -> None:
    ...
    assert 'const retryConfirm = adminGenericText("jobAction.retryConfirm", "Retry job {jobId}?' in PORTAL
    assert 'data-portal-confirm="${safeText(retryConfirm)}"' in PORTAL
