...
def test_features_renders_intent_led_guided_start_without_an_execution_claim() -> None:
    ...
    assert catalog.index("${renderFeatureFamilyExplorer()}") < catalog.index("${body}")
    assert catalog.index("${body}") < catalog.index("${renderFeatureGuidedStart(context)}${renderCapabilityHub(context)}")
