...
def test_light_account_activity_final_surface_keeps_owner_scoped_log_clear() -> None:
    """The private Web activity log stays readable without implying external history."""

    theme_source = PORTAL_THEME.read_text(encoding="utf-8")
    layer = re.search(
        r"/\* Final light Account Activity surface \*/(?P<css>.*?)(?=/\* Final light [^*]*\*/|\Z)",
        theme_source,
        flags=re.DOTALL,
    )

    assert layer is not None
    activity_css = layer.group("css")
    root_scope = ".portal-page.portal-account-activity"
    required = (
        ".portal-settings-nav",
        ".portal-campaign-boundary",
        ".portal-state",
    ...
    for evidence in required:
        assert evidence in activity_css

    def at_rule_body(at_rule: str) -> str:
        start = activity_css.index(at_rule)
        opening_brace = activity_css.index("{", start)
        depth = 0
        for index in range(opening_brace, len(activity_css)):
            if activity_css[index] == "{":
                depth += 1
            elif activity_css[index] == "}":
                depth -= 1
                if depth == 0:
                    return activity_css[opening_brace + 1 : index]
        raise AssertionError(f"unclosed {at_rule}")

    def declarations_for(selector: str, css: str = activity_css) -> str:
        rule = re.search(
            rf"{re.escape(selector)}\s*\{{(?P<declarations>[^}}]*)\}}",
            css,
            flags=re.DOTALL,
    ...
    assert "background: var(--portal-surface-light);" in declarations_for(
        f"{root_scope} .portal-data-table-wrap"
    )
    assert "background: var(--portal-surface-soft);" in declarations_for(
        f"{root_scope} .portal-notice"
    )
    assert "transform: none;" in declarations_for(
        f"{root_scope} .portal-button--quiet:is(:hover, :focus-visible)"
    )
    ...
    selectors = _final_light_layer_selectors(activity_css)
    assert selectors
    assert all(selector.startswith(root_scope) for selector in selectors)
    assert not re.search(r"#[0-9a-fA-F]{3,8}\b", activity_css)
    assert not re.search(r"\b(?:rgba?|hsla?)\(", activity_css.lower())
    assert "linear-gradient" not in activity_css.lower()
    assert "radial-gradient" not in activity_css.lower()
...
def test_light_admin_security_access_final_surface_keeps_aggregate_boundary_clear() -> None:
    """Security and access stay aggregate-only posture views, not a control plane."""

    portal_source = PORTAL_CLIENT
    start = portal_source.index("  function renderAdminSecurityAccessPosture(")
    next_renderer = portal_source.find("\n  function ", start + 1)
    renderer_source = portal_source[start : next_renderer if next_renderer >= 0 else None]
    assert '<article class="portal-page portal-admin-security-access-posture">' in renderer_source
    ...
    assert layer is not None
    rules = _parse_css_rules(layer.group("css"))
    root_scope = ".portal-page.portal-admin-security-access-posture"

    def assert_declarations(selector: str, expected: dict[str, str], *, at_rule: str | None = None) -> None:
        declarations = _css_declarations_for(rules, selector, at_rule=at_rule)
        assert {name: declarations.get(name) for name in expected} == expected
    ...
    assert_declarations(
        f"{root_scope} .portal-operations-metrics .portal-metric",
        {"background": "var(--portal-surface-light)", "box-shadow": "none"},
    )
    assert_declarations(f"{root_scope} .portal-card", {"background": "var(--portal-surface-light)", "box-shadow": "none"})
    assert_declarations(
        f"{root_scope} .portal-operations-run",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-light)"},
    )
    assert_declarations(
        f"{root_scope} .portal-operations-boundary",
        {
            "background": "var(--portal-surface-soft)",
            "border-color": "var(--portal-border)",
            "box-shadow": "none",
    ...
    assert_declarations(f"{root_scope} .portal-button--quiet:is(:hover, :focus-visible)", {"transform": "none", "box-shadow": "none"})
    assert_declarations(f"{root_scope} .portal-button:disabled", {"background": "var(--portal-surface-soft)", "color": "var(--portal-muted)"})
    assert_declarations(
        f"{root_scope} :is(button, a, input, select, textarea):focus-visible",
        {"outline": "3px solid var(--portal-focus) !important"},
    )
    assert_declarations(f'{root_scope} .portal-badge[data-status="read_only"]', {"color": "var(--portal-info)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="guarded"]', {"color": "var(--portal-warning)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="failed"]', {"color": "var(--portal-danger)"})

    medium = "@media (max-width: 980px)"
    assert_declarations(f"{root_scope} .portal-operations-admin-grid", {"grid-template-columns": "minmax(0, 1fr)"}, at_rule=medium)
    mobile = "@media (max-width: 700px)"
    assert_declarations(f"{root_scope} .portal-operations-metrics", {"grid-template-columns": "1fr"}, at_rule=mobile)
    assert_declarations(f"{root_scope} :is(.portal-button--primary, .portal-button--quiet)", {"min-height": "44px"}, at_rule=mobile)
    reduced_motion = "@media (prefers-reduced-motion: reduce)"
    assert_declarations(
        f"{root_scope} :is(.portal-card, .portal-metric, .portal-operations-run, .portal-button)",
        {"transition": "none", "transform": "none"},
        at_rule=reduced_motion,
    )
    ...
    def base_rule_index(selector: str) -> int:
        return next(
            index
            for index, rule in enumerate(rules)
            if rule["at_rules"] == () and selector in rule["selectors"]
        )

    assert base_rule_index(f"{root_scope} .portal-card") < base_rule_index(
        f"{root_scope} .portal-operations-boundary"
    )
    assert {rule["at_rules"] for rule in rules} == {(), (medium,), (mobile,), (reduced_motion,)}
    declaration_values = " ".join(value for rule in rules for value in rule["declarations"].values())
    assert not re.search(r"#[0-9a-fA-F]{3,8}\b", declaration_values)
    assert not re.search(r"\b(?:rgba?|hsla?)\s*\(", declaration_values, flags=re.IGNORECASE)
    assert not re.search(r"\b(?:repeating-)?(?:linear|radial|conic)-gradient\s*\(", declaration_values, flags=re.IGNORECASE)
    assert not re.search(r"(?<![-\w])transparent(?![-\w])", declaration_values, flags=re.IGNORECASE)
    assert not re.search(r"(?i:var)\(\s*--(?!portal-)", declaration_values)
    assert all(not name.startswith("--") or name.startswith("--portal-") for rule in rules for name in rule["declarations"])
...
def test_light_finance_operations_planning_final_surface_keeps_internal_planning_clear() -> None:
    """Signed admin planning remains dense, legible, and truthful on the light surface."""

    portal_source = PORTAL_CLIENT
    start = portal_source.index("  function renderAdminFinancePlanning(")
    next_renderer = portal_source.find("\n  function ", start + 1)
    renderer_source = portal_source[start : next_renderer if next_renderer >= 0 else None]
    assert '<article class="portal-page portal-finance-planning">' in renderer_source
    ...
    assert layer is not None
    finance_css = layer.group("css")
    rules = _parse_css_rules(finance_css)
    ...
    assert_declarations(
        f"{root_scope} .portal-card",
        {"background": "var(--portal-surface-light)", "box-shadow": "none"},
    )
    assert_declarations(
        f"{root_scope} .portal-data-table th",
        {"background": "var(--portal-surface-soft)", "color": "var(--portal-muted)"},
    )
    assert_declarations(
        f"{root_scope} .portal-data-table td",
        {"background": "var(--portal-surface-light)", "color": "var(--portal-ink)"},
    )
    assert_declarations(
        f"{root_scope} :is(.portal-input, .portal-select, .portal-textarea, .portal-form-note, [data-finance-planning-status])",
    ...
    assert_declarations(f'{root_scope} .portal-badge[data-status="ready"]', {"color": "var(--portal-success)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="processing"]', {"color": "var(--portal-info)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="guarded"]', {"color": "var(--portal-warning)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="failed"]', {"color": "var(--portal-danger)"})
    ...
    assert_declarations(
        f"{root_scope} :is(.portal-button--primary, .portal-button--quiet)",
        {"min-height": "44px"},
        at_rule=mobile,
    )
    ...
    reduced_motion = "@media (prefers-reduced-motion: reduce)"
    assert_declarations(
        f"{root_scope} :is(.portal-card, .portal-metric, .portal-button, .portal-input, .portal-select, .portal-textarea)",
        {"transition": "none", "transform": "none"},
        at_rule=reduced_motion,
    )
...
def test_light_operations_desk_final_surface_keeps_erp_queue_aligned_and_truthful() -> None:
    """The signed operations queue remains dense, legible, and truthful on the light surface."""
    ...
    assert layer is not None
    operations_css = layer.group("css")
    rules = _parse_css_rules(operations_css)
    root_scope = ".portal-page.portal-operations-desk"
    ...
    assert_declarations(
        f"{root_scope} .portal-support-filter",
        {
    ...
    assert_declarations(f'{root_scope} .portal-badge[data-status="read_only"]', {"color": "var(--portal-info)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="guarded"]', {"color": "var(--portal-warning)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="completed"]', {"color": "var(--portal-success)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="failed"]', {"color": "var(--portal-danger)"})
    ...
    reduced_motion = "@media (prefers-reduced-motion: reduce)"
    assert_declarations(
        f"{root_scope} :is(.portal-card, .portal-metric, .portal-support-filter, .portal-select, .portal-button, .portal-data-table tbody tr)",
        {"transition": "none", "transform": "none"},
        at_rule=reduced_motion,
    )
...
def test_light_admin_automation_monitor_final_surface_keeps_receipts_truthful() -> None:
    """The signed monitor stays a clear evidence surface, never an auto-fix claim."""

    portal_source = PORTAL_CLIENT
    start = portal_source.index("  function renderAdminAutomationMonitor(")
    next_renderer = portal_source.find("\n  function ", start + 1)
    renderer_source = portal_source[start : next_renderer if next_renderer >= 0 else None]
    assert '<article class="portal-page portal-admin-automation-monitor">' in renderer_source
    ...
    assert layer is not None
    monitor_css = layer.group("css")
    rules = _parse_css_rules(monitor_css)
    ...
    medium = "@media (max-width: 980px)"
    assert_declarations(
        f"{root_scope} .portal-operations-admin-grid",
        {"grid-template-columns": "minmax(0, 1fr)"},
        at_rule=medium,
    )
...
def test_light_reliability_followup_final_surface_keeps_actions_truthful() -> None:
    """Reliability remains a signed follow-up queue, not a fake auto-repair surface."""

    portal_source = PORTAL_CLIENT
    start = portal_source.index("  function renderReliabilityAdmin(")
    next_renderer = portal_source.find("\n  function ", start + 1)
    renderer_source = portal_source[start : next_renderer if next_renderer >= 0 else None]
    assert '<article class="portal-page portal-reliability-admin">' in renderer_source
    ...
    assert layer is not None
    reliability_css = layer.group("css")
    rules = _parse_css_rules(reliability_css)
    ...
    assert_declarations(
        f"{root_scope} .portal-reliability-row",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-light)"},
    )
    assert_declarations(
        f"{root_scope} .portal-operations-boundary",
    ...
    assert_declarations(
        f"{root_scope} .portal-reliability-action .portal-button:disabled",
        {"background": "var(--portal-surface-soft)", "color": "var(--portal-muted)"},
    )
    assert_declarations(
        f"{root_scope} :is(button, a, input, select, textarea):focus-visible",
    ...
    assert_declarations(f'{root_scope} .portal-badge[data-status="awaiting_confirm"]', {"color": "var(--portal-info)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="processing"]', {"color": "var(--portal-info)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="completed"]', {"color": "var(--portal-success)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="guarded"]', {"color": "var(--portal-warning)"})
    assert_declarations(f'{root_scope} .portal-badge[data-status="failed"]', {"color": "var(--portal-danger)"})
    ...
    reduced_motion = "@media (prefers-reduced-motion: reduce)"
    assert_declarations(
        f"{root_scope} :is(.portal-card, .portal-metric, .portal-reliability-row, .portal-support-filter, .portal-select, .portal-button)",
        {"transition": "none", "transform": "none"},
        at_rule=reduced_motion,
    )
...
def test_light_admin_audit_final_surface_keeps_redacted_receipts_clear() -> None:
    """Audit remains a signed, redacted evidence surface rather than a control plane."""

    portal_source = PORTAL_CLIENT
    start = portal_source.index("  function renderAdmin(page, context) {")
    next_renderer = portal_source.find("\n  function ", start + 1)
    renderer_source = portal_source[start : next_renderer if next_renderer >= 0 else None]
    ...
    assert layer is not None
    audit_css = layer.group("css")
    rules = _parse_css_rules(audit_css)
    ...
    assert_declarations(root_scope, {"color": "var(--portal-ink)", "background": "var(--portal-surface-light)"})
    assert_declarations(
        f"{root_scope} .portal-admin-guard",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-soft)", "box-shadow": "none"},
    )
    assert_declarations(
        f"{root_scope} .portal-work-grid > .portal-card",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-light)", "box-shadow": "none"},
    )
    assert_declarations(
        f"{root_scope} .portal-admin-audit-explorer",
        {"display": "grid", "gap": "16px"},
    )
    assert_declarations(
        f"{root_scope} .portal-admin-audit-explorer .portal-admin-grid .portal-metric",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-light)", "box-shadow": "none"},
    )
    assert_declarations(
        f"{root_scope} .portal-project-filter",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-soft)", "box-shadow": "none"},
    )
    assert_declarations(
        f"{root_scope} .portal-project-filter .portal-select",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-light)", "color": "var(--portal-ink)"},
    )
    assert_declarations(
        f"{root_scope} .portal-data-table th",
        {"background": "var(--portal-surface-soft)", "color": "var(--portal-muted)"},
    ...
    assert_declarations(
        f"{root_scope} .portal-support-pagination",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-soft)"},
    )
    assert_declarations(
        f"{root_scope} .portal-admin-audit-boundaries",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-soft)", "box-shadow": "none"},
    )
    assert_declarations(
        f"{root_scope} .portal-button--quiet:is(:hover, :focus-visible)",
    ...
    assert_declarations(
        f"{root_scope} :is(.portal-button, .portal-select):disabled",
        {"background": "var(--portal-surface-soft)", "color": "var(--portal-muted)"},
    )
    assert_declarations(
        f"{root_scope} :is(button, a, input, select, textarea):focus-visible",
    ...
    mobile = "@media (max-width: 700px)"
    assert_declarations(
        f"{root_scope} :is(.portal-admin-grid, .portal-project-filter .portal-fields)",
        {"grid-template-columns": "1fr"},
        at_rule=mobile,
    )
    assert_declarations(
        f"{root_scope} :is(.portal-button--primary, .portal-button--quiet, .portal-select)",
        {"min-height": "44px"},
        at_rule=mobile,
    )
    ...
    reduced_motion = "@media (prefers-reduced-motion: reduce)"
    assert_declarations(
        f"{root_scope} :is(.portal-card, .portal-metric, .portal-project-filter, .portal-data-table tbody tr, .portal-button)",
        {"transition": "none", "transform": "none"},
        at_rule=reduced_motion,
    )
    ...
    assert base_rule_index(f"{root_scope} .portal-work-grid > .portal-card") < base_rule_index(
        f"{root_scope} .portal-admin-audit-boundaries"
    )
    assert {rule["at_rules"] for rule in rules} == {
        (),
...
def test_light_admin_runtime_final_surface_keeps_queue_actions_truthful() -> None:
    """Runtime remains a canonical queue view, never a browser infrastructure console."""
    ...
    assert layer is not None
    runtime_css = layer.group("css")
    rules = _parse_css_rules(runtime_css)
    ...
    assert_declarations(
        f"{root_scope} .portal-admin-guard .portal-state",
        {"background": "var(--portal-surface-soft) !important", "box-shadow": "none"},
    )
    assert_declarations(
        f"{root_scope} .portal-work-grid > .portal-card",
        {"border-color": "var(--portal-border)", "background": "var(--portal-surface-light)", "box-shadow": "none"},
    ...
    assert_declarations(
        f"{root_scope} .portal-admin-write-actions",
    ...
    mobile = "@media (max-width: 700px)"
    assert_declarations(
        f"{root_scope} :is(.portal-button--primary, .portal-button--quiet, .portal-data-table .portal-button)",
        {"min-height": "44px"},
        at_rule=mobile,
    )
    ...
    reduced_motion = "@media (prefers-reduced-motion: reduce)"
    assert_declarations(
        f"{root_scope} :is(.portal-card, .portal-metric, .portal-data-table tbody tr, .portal-button)",
        {"transition": "none", "transform": "none"},
        at_rule=reduced_motion,
    )
