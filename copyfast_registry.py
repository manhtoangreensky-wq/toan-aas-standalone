...
CUSTOMER_FEATURES: tuple[WebFeature, ...] = (
    ...
    WebFeature("workspace_menu", "Chuyển workspace", "content", "/workspace-menu", description="Directory điều hướng Web-native đã review theo mục tiêu; không phát lại menu/callback Bot hoặc gọi bridge, provider, job, Xu hay PayOS."),
    ...
    WebFeature("community", "Cộng đồng", "account", "/community", description="Kênh chính thức, community và anti-impersonation guidance được Web App kiểm tra riêng; không replay state Bot."),
    ...
    WebFeature("audio_hub", "Audio Production Hub", "music", "/audio-hub", description="Bảng điều phối audio app-first dùng cùng collection Media Workspace và Asset Vault reference đã owner-check; không tạo database/API song song, player, provider, generator, job, Xu, PayOS hoặc output giả."),
    ...
    WebFeature("music_direction_presets", "Music Directions", "music", "/media-workspace/music-directions", description="Chọn một trong năm preset Web đã review rồi tạo ba hướng prompt nhạc deterministic dạng văn bản; preset không nhận callback/keyword Bot và không gọi provider, tạo lyrics/audio/preview/job, lưu Memory/asset/collection, thanh toán hoặc Telegram action."),
    WebFeature("sfx_cue_sheet", "SFX Cue Sheet Composer", "music", "/media-workspace/sfx-cue-sheet", description="Lập ba cue vị trí ngữ nghĩa cho opening/transition/closing từ một preset Web opaque và brief mới; không nhận callback/keyword Bot, không tạo mốc timeline, tìm catalog, preview/audio/output/job, lưu Memory/asset/collection, thanh toán hoặc Telegram action."),
    ...
    WebFeature("creative_motion_guide", "Creative Motion Guide", "video", "/video-studio/motion-guide", description="Chuyển flow Bot `motion` thành chọn nhóm/chủ đề, ba gợi ý có thể làm mới, style và motion guide dạng text deterministic; không dùng Telegram pending state, Image Studio, provider, media/preview/output/job, Xu/PayOS, asset, publish hoặc delivery."),
    ...
    WebFeature("cinematic_ad_concept", "Cinematic Ad Concept Composer", "video", "/video-studio/cinematic-concept", description="Compose từ brief chỉ là receipt tạm thời: tạo concept quảng cáo, ba hướng sáng tạo, storyboard và prompt deterministic, không lưu Bot state hay gọi provider/media/job/Xu/PayOS. Chỉ nút Lưu Plan được xác nhận riêng mới server-recompute input gốc để tạo Video Plan Draft Web-owned; không phải Bot save/lock/finalize, render, payment, asset, publish hoặc delivery."),
    ...
    WebFeature("subtitle_asset_operations", "Subtitle Asset Operations", "subtitle", "/subtitle/assets", description="Kiểm tra và chuyển đổi SRT/VTT hiện có trong Asset Vault riêng của signed owner; không nhận path/URL/text từ browser, không ASR, dịch, dubbing, provider, Bot, job, Xu hoặc PayOS."),
    WebFeature("audio_asset_operations", "Audio Asset Operations", "music", "/audio/assets", description="Kiểm định audio private, chuyển MP3/M4A hoặc chuẩn hóa M4A bằng profile cố định từ Asset Vault của signed owner; không nhận path, URL, codec hay lệnh xử lý từ browser và không gọi Bot/Core Bridge, provider, TTS, ASR, dubbing, Xu hoặc PayOS."),
    ...
    WebFeature("image_hub", "Image Operations Hub", "image", "/image-hub", description="Bảng điều phối app-first trình bày cùng artboard Image Creative Studio owner-scoped; không tạo database/API/engine riêng, không tự chuyển Asset Vault reference, gọi provider, tạo ảnh/preview/output, job, Xu hoặc PayOS."),
    WebFeature("quick_image_planner", "Quick Image Planner", "image", "/image/quick-planner", description="Chuyển luồng chọn gợi ý/tự nhập, biến thể prompt, logo/watermark text direction và tỷ lệ của Bot thành prompt plan deterministic để review; không tạo ảnh/preview, gọi provider/Bot/ShopAI, tier, job, Xu/PayOS, asset hoặc delivery."),
...
ADMIN_FEATURES: tuple[WebFeature, ...] = (
    ...
    WebFeature("admin_finance_planning", "Finance Operations Planning", "admin", "/admin/finance/planning", "admin", description="Ngân sách và kế hoạch chi phí vận hành Web-native có revision/audit; không phải ledger, payment, PayOS, Xu, doanh thu, refund, tax/export hay dữ liệu canonical của Bot."),
