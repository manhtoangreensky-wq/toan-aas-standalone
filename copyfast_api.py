...
from copyfast_video_ai_image_job_bridge import (
    CANONICAL_PRODUCT_KEY as VIDEO_AI_IMAGE_PRODUCT_KEY,
    create_or_replay_video_ai_image_job,
    get_video_ai_image_job,
    is_video_ai_image_job_other_account,
    list_video_ai_image_jobs,
    video_ai_image_job_to_native_compat,
)
...
FEATURE_TIER_REQUIRED_ON_CONFIRM = frozenset({
    ...
    "video_single", "video_ai_image", "video_product", "video_trend", "video_text_to_video", "video_quick",
...
def _flags() -> dict[str, bool]:
    ...
    return {
        ...
        # Partner Readiness is an isolated Web-native private profile. This
        # status flag exposes only readiness, never profile data or any
        # referral, matching, contact, payment, provider or Bot capability.
        "partner_readiness_enabled": enabled("WEBAPP_PARTNER_READINESS_ENABLED", True),
        ...
        # Frame Video Lab is a separate, local image-sequence MP4 boundary.
...
def _feature_input_contract_error(feature: str, values: dict[str, Any], *, action: str = "draft") -> str:
    ...
    if feature == "video_ai_image":
        from copyfast_video_ai_image_job_bridge import validate_video_ai_image_input
        if action == "confirm":
            is_valid, err, _ = validate_video_ai_image_input(values)
            if not is_valid:
                return err
        else:
            prompt = str(values.get("prompt") or values.get("text") or values.get("brief") or "").strip()
            ...
            if not source_img:
                ...
                if not (isinstance(uploads, (list, tuple)) and uploads):
                    return "upload_required"
...
def _native_jobs_for_account(account: dict) -> list[dict[str, Any]]:
    ...
    vai_jobs = [
        video_ai_image_job_to_native_compat(job)
        for job in list_video_ai_image_jobs(account_id, limit=100)
    ]
    return _merge_read_items(vai_jobs, _merge_read_items(pv_jobs, native_jobs))
...
def _payos_config() -> dict[str, str]:
    client_id = (os.environ.get("PAYOS_CLIENT_ID") or "").strip('\"').strip("'").strip()
    api_key = (os.environ.get("PAYOS_API_KEY") or "").strip('\"').strip("'").strip()
    checksum = (os.environ.get("PAYOS_CHECKSUM_KEY") or "").strip('\"').strip("'").strip()
    return {"client_id": client_id, "api_key": api_key, "checksum": checksum}


async def _create_payos_checkout(amount: int, order_code: int, description: str, public_base_url: str) -> dict[str, Any]:
    cfg = _payos_config()
    if not cfg["client_id"] or not cfg["api_key"] or not cfg["checksum"]:
        return {"code": "CONFIG_ERROR", "desc": "Cấu hình cổng PayOS chưa đầy đủ (thiếu client/api/checksum key)."}
    cancel_url = f"{public_base_url}/wallet/topup"
    return_url = f"{public_base_url}/wallet/topup"
    raw_str = f"amount={amount}&cancelUrl={cancel_url}&description={description}&orderCode={order_code}&returnUrl={return_url}"
    signature = hmac.new(cfg["checksum"].encode("utf-8"), raw_str.encode("utf-8"), hashlib.sha256).hexdigest()
    payload = {
...
async def job_detail(job_id: str, request: Request, account: dict = Depends(require_account)):
    ...
    vai_job = get_video_ai_image_job(account_id, job_id)
    if vai_job is not None:
        compat_item = video_ai_image_job_to_native_compat(vai_job)
        return envelope(
            True,
            "Đã tải dữ liệu Job Web-native của tài khoản hiện tại.",
            data={**compat_item, "job_record": vai_job, "read_model": "jobs", "canonical_available": False},
            status_name="read_only",
        )
    if is_video_ai_image_job_other_account(job_id, account_id):
        raise HTTPException(status_code=403, detail="Không có quyền truy cập job của tài khoản khác")
    if str(job_id or "").strip().startswith("vaij_"):
        return envelope(
            False,
            "Không tìm thấy Job Web-native thuộc tài khoản hiện tại.",
            status_name="guarded",
...
async def _feature_action(action: str, feature: str, payload: FeatureRequest, request: Request, account: dict, *, session_id: str = "") -> dict:
    ...
    if action == "confirm":
        ...
        if feature == "video_ai_image":
            account_id = str(account.get("id") or "")
            try:
                job_result = create_or_replay_video_ai_image_job(
                    account_id=account_id,
                    payload=values,
                    idempotency_key=key,
                )
                ...
                return envelope(
                    ...
                    exc.detail,
                    status_name="guarded",
                    error_code="VIDEO_AI_IMAGE_JOB_VALIDATION_FAILED",
                )
...
@router.post("/features/video_ai_image/jobs")
async def create_video_ai_image_job_route(
    payload: FeatureRequest,
    request: Request,
    account: dict = Depends(require_csrf),
):
...
@router.get("/features/video_ai_image/jobs")
async def list_video_ai_image_jobs_route(
    request: Request,
    account: dict = Depends(require_account),
):
    account_id = str(account.get("id") or "")
    jobs = list_video_ai_image_jobs(account_id, limit=100)
    return envelope(
        True,
...
@router.get("/features/video_ai_image/jobs/{job_id}")
async def get_video_ai_image_job_route(
    job_id: str,
    request: Request,
    account: dict = Depends(require_account),
):
    account_id = str(account.get("id") or "")
    job = get_video_ai_image_job(account_id, job_id)
    if job is not None:
        return envelope(
            True,
