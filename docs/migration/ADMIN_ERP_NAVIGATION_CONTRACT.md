...
| Web Local Admin | Signed Web session plus local `require_admin` | CRM Manager Directory, Automation Monitor, Governance Documents, Internal Document Archive, Security & Access Posture, Finance Operations Planning | Web-owned read models plus only narrowly documented local lifecycles. Finance Planning may persist a confirmed budget/cost plan with CSRF, revision, idempotency and audit; it never grants canonical Bot admin authority, session/MFA/role control, wallet/Xu, PayOS/payment, provider/job or ledger access. |
