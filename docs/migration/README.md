...
- Web source fingerprint: `7a333a9713e3126e67aae57dea91e789f1b7203247e69176c7c39f63f8412031`
...
- [`BILLING_MENU_CALLBACK_CONTRACT.md`](BILLING_MENU_CALLBACK_CONTRACT.md) — 5 exact Bot-admin Billing menu dispositions; each may only open a fresh canonical-admin payments read route and never becomes customer/manual top-up or a ledger/PayOS action.
- [`ADMIN_ERP_MENU_CALLBACK_CONTRACT.md`](ADMIN_ERP_MENU_CALLBACK_CONTRACT.md) — 15 exact Bot-admin category/status menu dispositions; each opens only a fresh canonical-admin Web read route and never transfers Bot snapshots, commands, provider/package/payment controls or runtime authority.
- [`FINANCE_ADD_EXPENSE_CALLBACK_CONTRACT.md`](FINANCE_ADD_EXPENSE_CALLBACK_CONTRACT.md) — 1 exact Bot Finance Add-Expense help disposition; it may only enter a fresh signed Web-local-admin Finance Planning workspace and never replays Bot help/command/pending/finance state or becomes a financial write.
...
- [`WEB_SUPPORT_ADVISOR_CONTRACT.md`](WEB_SUPPORT_ADVISOR_CONTRACT.md) — signed, read-only self-service preparation for one closed Web Support Desk category; it never creates a ticket, invokes a classifier/AI, calls Bot/Telegram, provider/job or payment/refund state.
- [`WEB_CONSULTATION_BRIEF_COMPOSER_CONTRACT.md`](WEB_CONSULTATION_BRIEF_COMPOSER_CONTRACT.md) — closed Premium/custom/service consultation catalog and non-persistent signed Web draft; explicit handoff only fills the normal Support form and never creates a lead, case, quote, contact, payment or Bot action.
- [`WEB_CONSULTATION_CRM_LEAD_CONTRACT.md`](WEB_CONSULTATION_CRM_LEAD_CONTRACT.md) — dedicated customer consultation request with closed catalog, non-persistent preview and explicit storage-only consent before one private Web CRM lead draft; it never grants contact, payment, provider, Bot or Support-case authority.
