...
| `docflow|send_more`, `reset_files`, `pop`, `clear`, `ask_pages`, `back`, `main` | **Source disposition, không phải callback parity** | Bot đọc/ghi `USER_PENDING`, Telegram file list và message navigation. Workspace có thể hỗ trợ authoring review mới, nhưng không nhận, xóa, pop hay khôi phục Bot pending state. |
| `docflow|compress|light|medium|strong` | **Semantic mismatch, không map runtime** | Web PDF Optimize chỉ có một profile structural đã kiểm chứng; không nhận hay quảng cáo tương đương ba nhãn nén của Bot. Một plan `optimize` chỉ có thể mở `/documents/compress` như handoff mới, không truyền PDF/profile. |
| `docflow|confirm`, `run` | **Bot execution/delivery boundary** | Không phải Web confirm/execute. Bot kiểm tra pending files/options rồi chạy/deliver theo state Bot; Workspace/plan không tạo request, charge, output, download hoặc success card. |
...
| `/pdf_to_word`, `/image_to_pdf`, `/pdf_to_images`, `/compress_pdf`, `/split_pdf`, `/merge_pdf` | **Existing deterministic separate tool** | Dùng các page `/documents/pdf-to-word`, `/documents/image-to-pdf`, `/documents/pdf-to-images`, `/documents/compress`, `/documents/split`, `/documents/merge` và `/api/v1/document-operations/*` hiện có. Workspace có thể hiển thị **closed navigation-only handoff** theo plan intent, nhưng không auto-call, không chia sẻ operation ID/output/state và không nói plan đã thực thi. Handoff không có query/prefill Asset Vault ID/file/page/profile/token; tool đích bắt buộc chọn input và kiểm tra session/CSRF/ownership/idempotency riêng. `/merge_pdf` của Bot từng planned vẫn được xử lý bằng Web operation độc lập có boundary riêng. Xem `DOCFLOW_CALLBACK_CONTRACT.md`. |
