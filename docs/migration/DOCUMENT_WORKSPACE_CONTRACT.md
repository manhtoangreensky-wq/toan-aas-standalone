...
| `/ocr_image`, `/ocr_pdf` | **Guarded/Telegram-only execution** | Workspace có operation label `ocr` để lập kế hoạch/QA, nhưng không đọc image/PDF, không gọi `pytesseract`, không tạo `.txt` và không charge. OCR engine chỉ có thể được thêm sau bằng module/contract riêng. |
