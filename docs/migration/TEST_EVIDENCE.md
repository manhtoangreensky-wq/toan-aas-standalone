...
| --- | --- | --- |
| Web App | `python -m pytest -q` | `180 passed, 1 warning` |
...
| Professional App Shell | route/session/static-contract tests + full Web suite | passed: anonymous `/` and `/app` redirect to `/login`; signed-but-unlinked users go to `/onboarding`; linked users go to `/dashboard`. Marketing is explicit at `/welcome`, not the application root. Railway checks `/health`. The dashboard hydrates only the signed account’s Web-owned drafts and combines them with existing owner-scoped canonical job/asset metadata; it creates no browser-owned job, payment, provider, ledger or delivery state. |
