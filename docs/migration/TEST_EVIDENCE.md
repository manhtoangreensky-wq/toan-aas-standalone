...
| --- | --- | --- |
| Static audit | Pinned Python 3.11 `scripts/migration/audit_bot_to_web.py` against the frozen Git snapshot and current Web source | 773 canonical Bot commands, 2,862 callback-data values and 703 Web routes; 38.69% static Web surface coverage, 83.91% safe disposition coverage, and runtime equivalence is `NOT_STATICALLY_VERIFIABLE` (0%). |
| Menu terminal catalog | Focused static audit tests plus generated `parity_gap.json` filter | All 61 observed P0 Menu records (28 unique literal callbacks and 11 unique templates) are `TELEGRAM_ONLY`; no Menu family remains in the feature-disposition backlog. This is a Bot-only source boundary, not a Web action or runtime-parity claim. |
| Workboard/Task terminal catalog | Focused static audit tests plus generated `parity_gap.json` filter | All 27 observed finite `pipe|*`/`task|*` records are `TELEGRAM_ONLY`; the source still exposes no Web route, browser action, Bot ID transfer, job/task mutation, provider call, output/delivery or ledger action. Case, suffix, unknown and future values remain source-review-required. |
| Ticket terminal catalog | Focused static audit tests plus generated `parity_gap.json` filter | All 20 observed finite `ticket|*` terminal records (17 unique raw literals) are `TELEGRAM_ONLY`; nine customer categories preserve the Telegram pending/record boundary and eight Bot-admin menu/list/search controls preserve the Bot-admin boundary. Case, whitespace, suffix, dynamic ID and unlisted values remain source-review-required. This is a Bot-only source boundary, not a Web action or runtime-parity claim. |
| Audit generator | `python -m py_compile scripts/migration/audit_bot_to_web.py` | passed locally; source parsing only. |
| Finance navigation contract | Pinned Python 3.11: `pytest -q tests/test_migration_audit.py::test_static_audit_preserves_finance_planning_authority_and_redacts_secret_literals tests/test_migration_audit.py::test_static_audit_keeps_admin_erp_menu_navigation_private_and_exact tests/test_finance_compliance_readiness_portal_contracts.py` | focused contracts passed; thirteen exact lower-case Finance source literals, including static custom-period guidance, open only fresh signed `/admin/finance` reads. The exact `menu|finance_compliance` status literal opens only fresh data-free readiness guidance; case/suffix variants, `finance_compliance_update`, and canonical tax-review boundaries stay fail-closed. |
...
| --- | --- | --- |
| Web App | `python -m pytest -q` | `208 passed, 1 warning` locally for the PDF Split slice |
...
| Project Center & Studio Documents | owner-scope/version/API/UI contract tests + local browser smoke | passed: a signed account without Telegram created a Project, added a Studio Document at v1, saved v2 and exposed version history. The API enforces CSRF, idempotency, owner scope, bounded content, secret rejection and optimistic revision conflicts; restore creates a new revision rather than deleting history. No Bot bridge, provider, PayOS or Xu path is invoked. |
| Asset Vault | owner-scope/upload/download/archive/API/UI contract tests + full Web suite | passed: a signed unlinked Web account can use `/asset-vault` without a Bot bridge. Streaming upload validates filename, MIME, magic/container and quota before an atomically promoted private blob; idempotency binds content and metadata, download rechecks owner/size/hash and sends attachment-only no-cache headers, and archive does not free retained-storage quota. No blob/path/hash is exposed to the browser or audit event; PWA does not cache Vault endpoints. |
| Project Package | immutable ZIP export/API/UI contract tests + full Web suite | passed: a signed unlinked Web account can export an owner-scoped Project snapshot as a private ZIP without Bot bridge/provider/payment access. The server records `queued → processing → completed` only after atomic storage and hash/size verification; replay returns the original immutable package, later document edits do not alter it, cross-account reads do not enumerate it, and a tampered artifact becomes unavailable rather than downloadable. ZIP/public responses/audit omit storage key, hash, path, account/Telegram identity, source blob and provider/payment data; PWA does not cache package APIs or attachments. |
| Web-native PDF Split | owner-scope/parser/output/API/UI contract tests + local desktop/mobile smoke | passed: a signed unlinked account selects only an active private Asset Vault PDF, then the Web server copies and integrity-checks it in isolated storage, enforces 20 MiB/30-page and contiguous-range bounds, strips interactive PDF page metadata, reparses/hash-checks the output, and exposes it only as a no-store attachment. Cross-account access, CSRF omission, archive/tamper/encrypted/oversize-page inputs, path/storage overlap, idempotency conflict and stale interrupted operations all fail closed without a Bot job, provider call, PayOS/webhook or Xu mutation. The PWA caches shell files only. |
| Studio authoring without Engine | portal static contracts + full Web suite | passed: on a draft-supported Studio route without a Web Engine capability, the primary submit action—including Enter—saves or updates the signed account’s Web workspace draft. The Bot companion is an optional, no-data shortcut only; it never receives prompt, upload, Telegram ID, quote, session, Xu or token. |
...
- The owner-scoped Campaign Planner detail page uses a strict UUID route and
  bounded `GET /api/v1/campaigns/{id}` projection. A missing or foreign plan
  produces the same guarded response and an honest empty Web view; it never
  probes, exposes, or substitutes a Bot campaign.
- Account Activity is a separate signed, owner-scoped projection of the
