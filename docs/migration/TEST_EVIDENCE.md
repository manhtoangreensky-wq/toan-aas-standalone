...
| --- | --- | --- |
| Static audit | Pinned Python 3.11 `scripts/migration/audit_bot_to_web.py` against the frozen Git snapshot and current Web main | 773 canonical Bot commands, 2,862 callback-data values and 664 Web routes; 38.71% static Web surface coverage, 80.46% safe disposition coverage, and runtime equivalence is `NOT_STATICALLY_VERIFIABLE` (0%). |
| Audit generator | `python -m py_compile scripts/migration/audit_bot_to_web.py` | passed locally; source parsing only. |
| Finance navigation contract | Pinned Python 3.11: `pytest -q tests/test_migration_audit.py::test_static_audit_preserves_finance_planning_authority_and_redacts_secret_literals tests/test_migration_audit.py::test_static_audit_keeps_admin_erp_menu_navigation_private_and_exact` | focused contracts passed; thirteen exact lower-case Finance source literals, including static custom-period guidance, open only fresh signed `/admin/finance` reads, while case/suffix/compliance and canonical tax-review boundaries stay fail-closed. |
