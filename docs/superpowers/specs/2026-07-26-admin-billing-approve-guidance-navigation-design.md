# Admin Billing Approve Guidance Navigation Design

## Decision

Map exactly one frozen Bot callback, `menu|admin_billing_duyet`, to the
...
Bot callback, Telegram identity, pending-deposit or bill ID, amount, payment
...
- `bot.py:122392-122399` defines the `duyet` page as static manual-command
  guidance: the Bot asks an operator to reconcile real money and then use
  `/duyet <bill_id> <Xu>`; it explicitly says approval does not happen from
  this menu callback.
- `bot.py:122434-122441` dispatches the literal only to that static guidance
  and a Telegram back keyboard.

`menu|admin_billing_tuchoi` and `menu|admin_billing_payos` remain
source-review-required. They describe, respectively, a manual rejection
command and PayOS test commands, and must not become browser payment,
approval, rejection, PayOS-test, webhook, or ledger actions without a
separate canonical contract.
...
2. **Browser approval form.** Rejected: it would need a new idempotent,
...
| --- | --- | --- | --- | --- |
| `menu|admin_billing_duyet` | `/admin/payments` | `SIGNED_CANONICAL_ADMIN_READ` | `NAVIGATION_ONLY` | Fresh read navigation only; no Bot bill/payment data, manual command, approval/rejection, PayOS test/webhook, wallet/Xu/ledger/provider/runtime state, or write authority crosses into Web. |
...
three exact Billing read navigations, the new Approval Guidance row, one fewer
unresolved menu callback, and no runtime-equivalence claim. The explanatory
contract must keep rejection, PayOS test, case variants, suffixes, and future
callbacks source-review-only.
