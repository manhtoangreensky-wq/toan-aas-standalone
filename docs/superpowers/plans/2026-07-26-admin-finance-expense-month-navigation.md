# Admin Finance Expense Month Navigation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Map the static Bot-admin Expense Month selector to fresh Admin
Finance read navigation without transferring finance data or enabling an
expense/write/export control.
...
period, category, add-expense, export, tax, and payment actions
source-review-only.
...
- Modify: `tests/test_migration_audit.py:155-180,1535-1665`
...
- [ ] **Step 2: Pin Expense Month dispositions and siblings**
...
the registry-derived count, `menu\\|finance_expense_month`, and literal
source-review exclusions for `menu|finance_expense` and
`menu|finance_add_expense`. Re-run the existing canonical-admin page
...
- Modify: `scripts/migration/audit_bot_to_web.py:1685-1780,11645-11675`
- Test: `tests/test_migration_audit.py`

- [ ] **Step 1: Add the literal to the private registry**

Add `menu|finance_expense_month` with:
...
Keep the registry-derived count. Add only the Expense Month read-navigation
wording plus the source-review exclusions for expense data, period/category,
and add-expense siblings. Preserve existing Revenue Month, package, and
provider boundaries.
...
```powershell
$tempRoot = Join-Path $env:TEMP ('toanaas-copyfast151-audit-' + [guid]::NewGuid().ToString('N'))
...
`/admin/finance` as `NAVIGATION_ONLY`; all expense data/period/category/add
siblings remain source-review-required.

- [ ] **Step 2: Curate semantic deltas only**

Copy only the Expense Month row/count/text and generated semantic totals.
Preserve unrelated checkout fingerprints, existing Revenue Month exclusions,
and the Audio Hub review-pack text.
...
read, expense write, export, payment, provider, webhook, or Railway change.

### Task 4: Commit and hand off the isolated slice

**Files:**

- Review: all files above plus the Expense Month spec and plan files

- [ ] **Step 1: Confirm scope**

Confirm no Bot file, public menu catalog, browser finance form/period/category
parameter, bridge/API route, finance calculation/expense write/export,
