...
BILLING_MENU_FRESH_WEB_ADMIN_NAVIGATION_ACTIONS: dict[str, dict[str, Any]] = {
    "menu|billing": {
        ...
        "source_evidence": (
            "The frozen Bot callback handler treats the exact billing action as admin-only, even where a "
            "top-up keyboard may render a billing button. Its protected branch contains Bot-local billing/manual-"
    ...
    "menu|admin_billing_duyet": {
        ...
        "source_evidence": (
            "The Bot-admin-only literal renders static manual-approval command guidance and a Telegram back keyboard. "
            "The Web opens a fresh canonical-admin Payments read route; it receives no Telegram identity, pending "
            "deposit, bill ID, amount, payment reference, transaction, wallet/Xu ledger, PayOS/webhook, provider, "
    ...
    "menu|admin_billing_tuchoi": {
        ...
        "source_evidence": (
            "The Bot-admin-only literal renders static manual-rejection command guidance and a Telegram back keyboard. "
            "The Web opens a fresh canonical-admin Payments read route; it receives no Telegram identity, pending "
            "deposit, bill ID, payment reference, transaction, wallet/Xu ledger, PayOS/webhook, provider, runtime "
            "or write authority."
        ),
    },
...
ADMIN_ERP_FRESH_WEB_NAVIGATION_ACTIONS: dict[str, dict[str, Any]] = {
    ...
    "menu|finance_expense_month": {
        ...
        "source_evidence": (
            "The Bot-admin-only literal renders static expense-period/add-expense guidance and a Telegram selector. "
    ...
    "menu|finance_profit": {
        ...
        "source_evidence": (
            "The Bot-admin-only literal renders static profit-period guidance and a Telegram selector. The Web opens "
            "a fresh canonical-admin Finance read route; it receives no Telegram identity, profit summary, period/"
            "report parameter, transaction, ledger/Xu, payment, PayOS, export/file, provider, runtime or write "
            "authority."
        ),
    },
    "menu|finance_export": {
        ...
        "source_evidence": (
            "The Bot-admin-only literal renders static export-command guidance and a Telegram selector. The Web opens "
...
MENU_ACTION_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    # This is only the Bot's public profile hint.  It opens a fresh signed
    # Web account page and never carries a Telegram profile/referral identity,
    # link, reward record or Bot-local context into the browser.
    "menu|hint_profile": {
        "capability_key": "account",
        "target": "/account",
        "feature_key": "account",
    ...
    # The Bot hint is a customer-facing entry only.  The Web route still
    # repeats its payment flag, signed-session and canonical bridge checks;
    # this mapping never accepts a Bot amount/order, creates a manual top-up,
    # credits Xu or finalizes PayOS/webhook state in the browser.
    "menu|hint_naptien": {
        "capability_key": "wallet_topup",
        "target": "/wallet/topup",
        "feature_key": "wallet_topup",
...
# The frozen Image Tools editor uses one literal parameter default as a back
# button callback.  The large-file extractor cannot resolve generic callback
# variables, so keep this source-only derivation deliberately narrower: only
# a reviewed default literal that is passed unchanged as ``callback_data`` in
# its own function may be recorded.
IMGTOOL_DEFAULT_CALLBACK_FUNCTION_RE = re.compile(
    r"(?ms)^(?P<indent>[ \t]*)(?:async\s+)?def\s+(?P<function>[A-Za-z_]\w*)\s*\("
    r"(?P<parameters>.*?)\)\s*(?:->[^:\r\n]+)?[ \t]*:[ \t]*(?:\r?\n)?"
    r"(?P<body>.*?)(?=^(?P=indent)(?:async\s+)?def\s|\Z)"
)
IMGTOOL_LITERAL_DEFAULT_CALLBACK_PARAMETER_RE = re.compile(
    r"(?<![A-Za-z0-9_])(?P<parameter>[A-Za-z_]\w*)"
    r"(?:\s*:\s*[^=,\r\n]+)?\s*=\s*"
    r"(?P<quote>['\"])(?P<token>[^'\"\r\n]+)(?P=quote)"
)
IMGTOOL_DIRECT_CALLBACK_PARAMETER_RE = re.compile(
    r"\bcallback_data\s*=\s*(?P<parameter>[A-Za-z_]\w*)\b(?=\s*(?:[,\)\]\r\n#]|$))"
)
IMGTOOL_NESTED_FUNCTION_RE = re.compile(
...
def _resolve_reviewed_imgtool_default_callbacks(
    *,
    text: str,
    root: Path,
    path: Path,
    ...
    """Record the frozen reviewed Image Tools default callback without evaluation.

    This is intentionally not a general variable resolver.  A callback is
    added only when its quoted default is already in the finite Image Tools
    navigation allow-list and the exact parameter name appears directly as a
    ``callback_data`` keyword in that same function body.  It does not inspect
    callers, aliases, reassignment, templates or other callback namespaces. A
    nested function starts an opaque scope boundary, so its callback uses (and
    later source) are not attributed to the enclosing function.
    """

    relative_path = _relative(path, root)
    for function_match in IMGTOOL_DEFAULT_CALLBACK_FUNCTION_RE.finditer(text):
        parameters = str(function_match.group("parameters") or "")
        body = str(function_match.group("body") or "")
        body_offset = function_match.start("body")
        nested_function = IMGTOOL_NESTED_FUNCTION_RE.search(body)
        direct_matches = [
            match
            for match in IMGTOOL_DIRECT_CALLBACK_PARAMETER_RE.finditer(body)
            if nested_function is None or match.start() < nested_function.start()
        ]
        direct_parameters = {str(match.group("parameter") or "") for match in direct_matches}
        if not direct_parameters:
            continue

        for default_match in IMGTOOL_LITERAL_DEFAULT_CALLBACK_PARAMETER_RE.finditer(parameters):
            parameter = str(default_match.group("parameter") or "")
            token = str(default_match.group("token") or "")
            if parameter not in direct_parameters or token not in IMGTOOL_FRESH_WEB_NAVIGATION_ACTIONS:
                continue
            direct_match = next(
                (
                    match
            ...
            record = {
                "token": token,
                "resolution": "reviewed_imgtool_literal_default_callback_parameter",
                "function": str(function_match.group("function") or ""),
                "callback_parameter": parameter,
                "default_callback": token,
                "file": relative_path,
                "line": _line_for_offset(text, body_offset + direct_match.start()),
            }
            _append_unique(callback_data, seen["callback_data"], record, ("token", "file", "line"))
...
def _extract_python_inventory(root: Path, files: list[Path]) -> dict[str, Any]:
    ...
    for path in files:
        ...
        _resolve_reviewed_imgtool_default_callbacks(
            text=_read_source(path),
            root=root,
            path=path,
            callback_data=callback_data,
...
IMGTOOL_FRESH_WEB_NAVIGATION_ACTIONS: dict[str, dict[str, str]] = {
    ...
    "imgtool|edit_need_image": {
        "target": "/image/edit",
        "web_image_tool_intent": "web_image_enhance",
    },
    "imgtool|resize_need_image": {
        "target": "/image/resize",
    ...
    "imgtool|resize_task|pixels": {
        "target": "/image/resize",
        "web_image_tool_intent": "web_image_resize",
    },
    "imgtool|resize_method|blur": {
        "target": "/image/resize",
        "web_image_tool_intent": "web_image_resize",
    },
    ...
    "imgtool|editor_preset|product_clean": {
        "target": "/image/edit",
        "web_image_tool_intent": "web_image_enhance",
    },
    "imgtool|editor_preset|cinematic_warm": {
        "target": "/image/edit",
        "web_image_tool_intent": "web_image_enhance",
    },
    "imgtool|editor_preset|fresh_blue": {
        "target": "/image/edit",
        "web_image_tool_intent": "web_image_enhance",
    },
    "imgtool|editor_preset|food_vivid": {
        "target": "/image/edit",
        "web_image_tool_intent": "web_image_enhance",
    },
    "imgtool|editor_overlays": {
        "target": "/image/brand-overlay",
        "web_image_tool_intent": "web_image_brand_overlay",
    },
    "imgtool|editor_text": {
        "target": "/image/brand-overlay",
        "web_image_tool_intent": "web_image_brand_overlay",
...
def _imgtool_source_review_mapping(
    identifier: str,
    source_kind: str,
    evidence: dict[str, Any],
) -> dict[str, Any]:
    """Keep Bot Image Tools state transitions out of generic Web navigation.

    The frozen ``imgtool|*`` handler is a broad Telegram state machine. Its
    callbacks can consume a Telegram user/file/result/pending record, mutate
    Bot-local prompt or memory state, enter canonical ShopAI/Xu/provider
    paths, create a local output, or send an output back to Telegram. There
    stays source-review-required unless a finite exact entry is reviewed below.
    """

    return {
        "source_kind": source_kind,
        "source": identifier,
        "target": "IMGTOOL_SOURCE_REVIEW_REQUIRED",
        "classification": "customer",
        "status": "NEEDS_FEATURE_DISPOSITION",
        "resolution": "imgtool_callback_requires_web_native_image_workflow_contract",
        "source_dispositions": (
            "TELEGRAM_IDENTITY_CONTEXT",
            "BOT_IMAGE_TOOL_PENDING_STATE",
            "BOT_TELEGRAM_FILE_RESULT_OR_MEMORY_STATE",
            "CANONICAL_BOT_SHOPAI_PROVIDER_WALLET_OR_DELIVERY_BOUNDARY",
            "SOURCE_STATE_MACHINE_REQUIRED",
            "NO_WEB_NAVIGATION_OR_BROWSER_ACTION",
            "NO_BOT_FILE_OR_PENDING_STATE_REPLAY",
            "NO_PROVIDER_JOB_WALLET_PAYMENT_OR_DELIVERY_ACTION",
            "NO_RUNTIME_CLAIM",
        ),
        "source_evidence": (
            "The frozen Bot Image Tools handler reads the Telegram caller and per-user image-tool pending/result "
            "state. Its branches can select a Telegram file, generate or save a prompt/note, perform local edit/resize "
            "output, consume a ShopAI confirmation, select a tier, call an image-edit provider or deliver media in Telegram. "
            "The standalone Web has no adapter that accepts or replays an imgtool callback, Telegram file id, pending "
            "state, confirmation, charge, provider response or output."
        ),
        "evidence": evidence,
    }


def _map_imgtool_callback(
    identifier: str,
    source_kind: str,
    evidence: dict[str, Any],
    existing_routes: set[str],
...
# never placed in a URL, form, storage, API request or Web support record.
#
# The opaque ``web_support_intent`` values are review labels for this auditor
# alone. They deliberately do not preselect a Web category or create a case;
# the customer must make that choice and submit a new owner-scoped form in the
# independently authorized Web Support Desk.
FEEDBACK_FRESH_WEB_SUPPORT_NAVIGATION_ACTIONS: dict[str, dict[str, str]] = {
    "feedback|start": {
        "target": "/support",
        "web_support_intent": "web_support_start",
    },
    "feedback|cat|payment_topup": {
        "target": "/support",
        "web_support_intent": "web_support_billing_inquiry",
    },
    "feedback|cat|image_error": {
        "target": "/support",
        "web_support_intent": "web_support_image_help",
    },
    # This is only a Support Desk taxonomy cue. It neither opens a Video
    # route nor claims a video job, render, output or provider capability.
    "feedback|cat|video_error": {
        "target": "/support",
        "web_support_intent": "web_support_video_help",
    },
    "feedback|cat|document_pdf": {
        "target": "/support",
        "web_support_intent": "web_support_document_help",
    },
    "feedback|cat|package_combo": {
        "target": "/support",
        "web_support_intent": "web_support_package_help",
    },
    "feedback|cat|refund": {
        "target": "/support",
        "web_support_intent": "web_support_account_review",
    },
    "feedback|cat|feature_request": {
        "target": "/support",
        "web_support_intent": "web_support_product_feedback",
    },
    "feedback|cat|other": {
        "target": "/support",
        "web_support_intent": "web_support_general",
    },
}
...
SUPPORT_TICKET_FRESH_WEB_NAVIGATION_ACTIONS: dict[str, dict[str, str]] = {
    "support|start": {
        "target": "/support",
        "web_support_ticket_intent": "web_support_start",
    },
    "support|consult": {
        "target": "/support",
        "web_support_ticket_intent": "web_support_consultation",
    },
    "support|premium": {
        "target": "/support",
        "web_support_ticket_intent": "web_support_premium_consultation",
    },
    "support|admin_contact": {
        "target": "/support",
        "web_support_ticket_intent": "web_support_admin_contact",
    },
    "ticket|start": {
        "target": "/support",
        "web_support_ticket_intent": "web_support_ticket_start",
    },
    "support|ticket": {
        "target": "/support",
        "web_support_ticket_intent": "web_support_ticket_create",
    },
    "ticket|mine": {
        "target": "/tickets",
        "web_support_ticket_intent": "web_ticket_history",
    },
}


def _feedback_fresh_web_support_navigation_mapping(
    identifier: str,
    source_kind: str,
    evidence: dict[str, Any],
    existing_routes: set[str],
) -> dict[str, Any] | None:
    """Map only the finite feedback menu literals to a new Web intake.

    Exact lookup is intentional: Bot case variants, unknown categories,
    suffixes, extra segments and dynamic templates must continue through the
    source-review boundary below rather than inheriting a Web route.
    """

    action = FEEDBACK_FRESH_WEB_SUPPORT_NAVIGATION_ACTIONS.get(identifier)
    if action is None:
        return None
    target = str(action["target"])
    return {
        ...
        "classification": "customer",
        "status": _mapping_status(target, existing_routes, telegram_only=False, navigation_only=True),
        "resolution": "reviewed_feedback_fresh_web_support_navigation",
        "source_dispositions": (
            "FRESH_SIGNED_WEB_SUPPORT_NAVIGATION",
        ...
        "source_evidence": (
            "The exact frozen Bot feedback menu item may open a new, signed Web Support Desk form only. "
            "The Web never receives the Bot callback or feedback category, clears or creates no Bot pending state, "
            "and does not invoke the Bot classifier/ticket/admin-alert path, Telegram delivery, refund, payment, "
            "wallet/Xu, provider, job or runtime action."
        ),
        "feedback_support_authority": "SIGNED_WEB_NATIVE_CUSTOMER",
        "feedback_support_launch_mode": "WEB_NAVIGATION",
        "web_support_intent": str(action["web_support_intent"]),
        "evidence": evidence,
    }


def _support_ticket_fresh_web_navigation_mapping(
    identifier: str,
    source_kind: str,
    evidence: dict[str, Any],
    existing_routes: set[str],
) -> dict[str, Any] | None:
    """Map only the seven exact Support/Ticket entries to fresh Web navigation.

    The case-sensitive lookup intentionally rejects Bot case variants, suffixes,
    extra segments and dynamic templates so they remain source-review-required.
    """

    action = SUPPORT_TICKET_FRESH_WEB_NAVIGATION_ACTIONS.get(identifier)
    if action is None:
        return None
    target = str(action["target"])
    return {
        ...
        "source_evidence": (
            "The exact frozen Bot Support/Ticket entry may open a fresh signed Web route only. "
            "The Web receives no raw Bot callback, Telegram identity, ticket/lead/attachment identifier, category, "
            "pending input or Bot record state, and it initiates no Bot, provider, payment, wallet, job or Telegram "
            "delivery effect."
        ),
...
def _support_ticket_source_review_mapping(
    ...
    pending_dispositions = (
        ("BOT_FEEDBACK_CATEGORY_AND_PENDING_TEXT_STATE",)
        if is_feedback
        else ("BOT_SUPPORT_TICKET_PENDING_OR_RECORD_STATE",)
    )
    role_dispositions = (
        (
            "BOT_ADMIN_ONLY",
    ...
    return {
        ...
        "source_evidence": (
            "The frozen Bot feedback handler binds a Telegram user to a selected category and short-lived pending text; "
            "its next text can classify/create a Bot support ticket and alert an admin, including a refund-review request. "
            "The standalone Web has no adapter that accepts or replays the callback, category, pending text, Bot ticket "
            "record, admin alert or Telegram delivery state."
            if is_feedback
            else "The frozen Bot support and ticket handlers bind callbacks to a Telegram user and short-lived support "
...
def _map_support_ticket_callback(
    ...
    """Map finite feedback intake navigation; fail closed for every other action."""

    fresh_feedback_navigation = _feedback_fresh_web_support_navigation_mapping(
        identifier,
        source_kind,
        evidence,
        existing_routes,
    )
    if fresh_feedback_navigation is not None:
        return fresh_feedback_navigation

    fresh_support_ticket_navigation = _support_ticket_fresh_web_navigation_mapping(
        identifier,
        source_kind,
        evidence,
        existing_routes,
...
def _creative_variant_source_review_mapping(
    identifier: str,
    source_kind: str,
    evidence: dict[str, Any],
) -> dict[str, Any]:
    """Keep Bot-admin Creative variant selection out of generic Web routes."""

    return {
        "source_kind": source_kind,
        "source": identifier,
        "target": "CREATIVE_VARIANT_SOURCE_REVIEW_REQUIRED",
        "classification": "admin",
        "status": "NEEDS_FEATURE_DISPOSITION",
        "resolution": "creative_variant_callback_requires_web_native_owner_role_contract",
        "source_dispositions": (
            "TELEGRAM_IDENTITY_CONTEXT",
            "BOT_ADMIN_ONLY",
            "BOT_CREATIVE_VARIANT_IDENTIFIER",
            "CANONICAL_BOT_CREATIVE_VARIANT_SELECTION_AND_PRODUCTION_STATE",
            "SOURCE_STATE_MACHINE_REQUIRED",
            "NO_WEB_NAVIGATION_OR_BROWSER_ACTION",
            "NO_CREATIVE_VARIANT_OR_HANDOFF_STATE_REPLAY",
            "NO_PROVIDER_JOB_OUTPUT_OR_DELIVERY_ACTION",
            "NO_WALLET_PAYMENT_REFUND_OR_LEDGER_ACTION",
            "NO_RUNTIME_CLAIM",
        ),
        "source_evidence": (
            "The frozen Bot `creative|*` handler requires the configured Telegram admin, accepts only a select "
            "action with an opaque creative-variant ID, resolves it against the Bot owner, marks the selected variant "
            "and resets sibling variants, updates canonical production job stage/status/note, then renders a Telegram "
            "handoff instruction. The standalone Web has no adapter that accepts or replays the callback, Bot variant "
            "ID, selected state, production-job change, handoff instruction or Telegram admin context."
        ),
        "evidence": evidence,
    }


def _map_creative_variant_callback(
    identifier: str,
    source_kind: str,
    evidence: dict[str, Any],
) -> dict[str, Any] | None:
    """Fail closed for every Bot Creative callback until Web-native flows exist."""

    if not str(identifier or "").casefold().startswith("creative|"):
        return None
...
def _map_callback(identifier: str, source_kind: str, evidence: dict[str, Any], existing_routes: set[str]) -> dict[str, Any]:
    ...
    imgtool_mapping = _map_imgtool_callback(identifier, source_kind, evidence, existing_routes)
    if imgtool_mapping is not None:
        return imgtool_mapping
    support_ticket_mapping = _map_support_ticket_callback(identifier, source_kind, evidence, existing_routes)
    ...
    # Bot callback payloads are case-sensitive.  Unlike the older broad
    # classifier heuristics below, this private contract is deliberately an
    ...
    if billing_menu_navigation_entry is not None:
        ...
        # a new independently authorized Admin ERP *read navigation*; neither
        # a customer top-up nor a payment/ledger/PayOS operation is replayed.
        target = str(billing_menu_navigation_entry["target"])
        return {
            "source_kind": source_kind,
            "source": identifier,
            "target": target,
            "classification": str(billing_menu_navigation_entry["classification"]),
            "status": _mapping_status(target, existing_routes, telegram_only=False, navigation_only=True),
            "resolution": "reviewed_billing_menu_admin_navigation",
            "source_dispositions": tuple(billing_menu_navigation_entry["source_dispositions"]),
            "source_evidence": str(billing_menu_navigation_entry["source_evidence"]),
...
def _map_callback_template(template: str, evidence: dict[str, Any], existing_routes: set[str]) -> dict[str, Any] | None:
    ...
    if provider_choice_mapping is not None:
        return provider_choice_mapping
    imgtool_mapping = _map_imgtool_callback(template, "callback_template", evidence, existing_routes)
    if imgtool_mapping is not None:
        return imgtool_mapping
    support_ticket_mapping = _map_support_ticket_callback(template, "callback_template", evidence, existing_routes)
    if support_ticket_mapping is not None:
        return support_ticket_mapping
    ...
    if workboard_task_mapping is not None:
        return workboard_task_mapping
    creative_variant_mapping = _map_creative_variant_callback(template, "callback_template", evidence)
    if creative_variant_mapping is not None:
        return creative_variant_mapping
...
def _render_docs(docs_dir: Path, preflight: dict[str, Any], bot: dict[str, Any], web: dict[str, Any], gap: dict[str, Any]) -> list[Path]:
    ...
    imgtool_fresh_navigation_contract_rows = [
        [
            source,
            str(contract["target"]),
    ...
    imgtool_contract_rows = imgtool_fresh_navigation_contract_rows + [
        [
            ...
            "imgtool_callback_requires_web_native_image_workflow_contract",
            "Telegram user plus pending image/file/result/prompt/memory state; branches can enter local output, ShopAI tier/confirmation, canonical Xu/provider and Telegram delivery",
        ],
        [
            "case variants, missing tokens, suffixes or future imgtool|* values",
            "IMGTOOL_SOURCE_REVIEW_REQUIRED",
            "imgtool_callback_requires_web_native_image_workflow_contract",
    ...
    feedback_menu_contract_rows = [
        [
            source,
            str(contract["target"]),
            "reviewed_feedback_fresh_web_support_navigation",
            "NAVIGATION_ONLY",
            "SIGNED_WEB_NATIVE_CUSTOMER",
            str(contract["web_support_intent"]),
            "FRESH_SIGNED_WEB_SUPPORT_NAVIGATION, FINITE_BOT_FEEDBACK_INTENT_ONLY, "
            "NO_RAW_BOT_CALLBACK_OR_CATEGORY_TO_BROWSER, BOT_FEEDBACK_PENDING_STATE_NOT_REPLAYED, "
            "BOT_CLASSIFIER_TICKET_ADMIN_ALERT_NOT_REPLAYED, WEB_EXPLICIT_CSRF_IDEMPOTENT_CASE_CREATION_ONLY, "
            "NO_TELEGRAM_TICKET_LEAD_ATTACHMENT_NOTIFICATION_PROVIDER_JOB_WALLET_PAYMENT_REFUND_LEDGER_ACTION, "
            "NO_RUNTIME_CLAIM",
        ]
        for source, contract in FEEDBACK_FRESH_WEB_SUPPORT_NAVIGATION_ACTIONS.items()
    ]
    support_ticket_contract_rows = [
        [
            source,
            str(contract["target"]),
            "reviewed_support_ticket_fresh_web_navigation",
            "finite fresh signed Web navigation only; no raw Bot callback/state, category preselection, Bot ticket fetch or case creation from the Bot entry",
        ]
        ...
        [
            ...
            "/support",
            "reviewed_feedback_fresh_web_support_navigation",
            "finite fresh signed Web Support Desk navigation only; no Bot callback/category/pending/ticket/attachment/classifier/admin-alert or external-action replay",
        ],
        [
            "all unlisted support|* literals and templates",
        ...
        [
            "all unlisted customer ticket|* literals and templates",
            "SUPPORT_TICKET_SOURCE_REVIEW_REQUIRED",
            "support_ticket_callback_requires_web_native_owner_role_contract",
            "Telegram user plus customer-owned Bot ticket, reply/attachment pending state and Bot ticket identifier",
        ],
        [
            "all other feedback|* literals and templates, including cancel, unknown category, case/suffix/extra-segment variants",
            "SUPPORT_TICKET_SOURCE_REVIEW_REQUIRED",
            "support_ticket_callback_requires_web_native_owner_role_contract",
            "Telegram user plus selected feedback category and pending text; the next Bot message can classify/create a support ticket and alert an admin, including refund review",
        ],
        [
        ...
        [
            "case variants, missing tokens, suffixes or future support|*/ticket|*/feedback|* values",
            "SUPPORT_TICKET_SOURCE_REVIEW_REQUIRED",
            "support_ticket_callback_requires_web_native_owner_role_contract",
    ...
    creative_variant_contract_rows = [
        [
            "all frozen creative|select|* literals and templates",
            "CREATIVE_VARIANT_SOURCE_REVIEW_REQUIRED",
            "creative_variant_callback_requires_web_native_owner_role_contract",
            "Telegram admin plus opaque Bot creative-variant ID; selects one canonical variant, clears sibling selection and updates production job script/working state",
        ],
        [
            "case variants, missing tokens, suffixes or future creative|* values",
            "CREATIVE_VARIANT_SOURCE_REVIEW_REQUIRED",
            "creative_variant_callback_requires_web_native_owner_role_contract",
            "no Web Content Studio/Admin route, browser navigation/reset, Bot creative/job/handoff replay, provider/output/delivery/ledger action or runtime claim",
        ],
    ]
    ...
    write(
        ...
        + "- [`IMAGE_TOOLS_CALLBACK_CONTRACT.md`](IMAGE_TOOLS_CALLBACK_CONTRACT.md) — eighteen exact Bot Image Tools picker literals may only begin fresh signed Web-native Image routes; no Telegram pending state/file/result, ShopAI/Xu/provider branch or Telegram delivery becomes a browser input or runtime action, and every unlisted callback remains source-review-required.\n"
        + "- [`SUPPORT_TICKET_CALLBACK_CONTRACT.md`](SUPPORT_TICKET_CALLBACK_CONTRACT.md) — Bot Support/Ticket/Feedback callbacks remain owner/role/source-review boundaries apart from seven exact Support/Ticket entries and nine exact Feedback entries that may only start fresh signed Web navigation; no Telegram ticket, lead, attachment, pending text, admin preview or delivery state becomes a Web portal action.\n"
    ...
    write(
        ...
        "- Bot Image Tools callbacks are a Telegram state-machine boundary: `imgtool|*` can use pending/result/file/prompt/note state, local output, ShopAI tier/confirmation, provider/Xu and Telegram delivery. Eighteen exact picker literals may only begin fresh signed Web-native Image navigation with no source-state transfer; every other value must not route or replay them. See `IMAGE_TOOLS_CALLBACK_CONTRACT.md`.\n"
