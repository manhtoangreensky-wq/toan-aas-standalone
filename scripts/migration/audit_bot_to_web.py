...
STANDARD_WEB_AUDIT_EXCLUDED_RELATIVE_DIRS = (
    *STANDARD_MIGRATION_OUTPUT_RELATIVE_DIRS,
    ...
    # The auditor is migration tooling, never an application runtime module.
...
def _support_ticket_fresh_web_navigation_mapping(
    ...
    """Map only the eleven exact Support/Ticket entries to fresh Web navigation.
...
def _render_docs(docs_dir: Path, preflight: dict[str, Any], bot: dict[str, Any], web: dict[str, Any], gap: dict[str, Any]) -> list[Path]:
    ...
    web_revision = preflight.get("webapp", {}).get("revision", {})
    web_checkout_sha = str(web_revision.get("checkout_sha") or "unavailable")
    web_requested_sha = str(web_revision.get("requested_sha") or "not_requested")
    web_requested_relation = str(web_revision.get("requested_relation") or "not_audited")
    web_worktree_state = str(web_revision.get("working_tree_state") or "not_audited")
    web_revision_summary = (
        f"- Web source audit snapshot: `{web_checkout_sha}`; requested revision `{web_requested_sha}` "
        f"(`{web_requested_relation}`); eligible-source worktree `{web_worktree_state}`\n"
    )
    ...
    write(
        ...
        + "- The Web source inventory excludes `docs/superpowers` planning documents, `scripts/migration` audit tooling and configured/generated evidence output roots; other eligible Web source remains in scope.\n"
        ...
        + "- [`AUDIO_OPERATION_ASSET_EXPORT_CONTRACT.md`](AUDIO_OPERATION_ASSET_EXPORT_CONTRACT.md) — opt-in copy of a verified completed Web-native MP3/M4A transform into one owner-scoped Asset Vault record; no Bot/provider/payment/Content Handoff write.\n"
...
def _web_inventory_excluded_roots(web_root: Path, *configured_roots: Path) -> tuple[Path, ...]:
    """Return the exact non-runtime roots omitted from a Web source inventory."""
    ...
    return tuple(
        dict.fromkeys(
            ...
            if path != web_root and _path_is_within(path.resolve(), web_root)
        )
    )
...
def _web_revision_context(
    ...
    context = {
        ...
        "requested_relation": "not_a_git_worktree",
        "working_tree_state": "not_a_git_worktree",
        "source_fingerprint_sha256": str(source_fingerprint_sha256 or ""),
    }
    head_status, head = _git_read(root, "rev-parse", "--verify", "HEAD")
    if head_status != 0 or not re.fullmatch(r"[0-9a-f]{40}", head):
        return context
    ...
    if not requested:
        ...
        return context
    if not re.fullmatch(r"[0-9a-f]{7,64}", requested):
        context["requested_relation"] = "requested_sha_invalid"
        return context
    requested_status, resolved_requested = _git_read(root, "rev-parse", "--verify", f"{requested}^{{commit}}")
...
def _read_json_object(path: Path, label: str) -> dict[str, Any]:
    """Read a generated evidence object without allowing malformed shapes through."""
...
def verify_web_evidence(
    ...
    preflight = _read_json_object(report_dir / "preflight.json", "Migration preflight evidence")
    web_inventory = _read_json_object(report_dir / "web_inventory.json", "Web inventory evidence")
...
def run_audit(
    bot_root: Path,
    web_root: Path,
    bot_baseline_sha: str,
    report_dir: Path,
    docs_dir: Path,
    web_revision_sha: str = "",
) -> dict[str, Any]:
    ...
    web_output_roots = _web_inventory_excluded_roots(web_root, report_dir, docs_dir)
    with _locked_bot_source_snapshot(bot_root, bot_baseline_sha) as (audit_bot_root, audit_source):
        ...
        preflight = {
            ...
            "guarantees": [
                ...
                "Migration audit tooling under scripts/migration is excluded from the Web source inventory.",
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--bot-root", type=Path, help="Telegram bot source root; read-only")
    parser.add_argument("--web-root", required=True, type=Path, help="Web App source root; read-only")
    parser.add_argument("--bot-baseline-sha", help="Already verified bot baseline SHA to record")
    parser.add_argument("--web-revision", required=True, help="Expected local Web Git revision SHA")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = run_audit(
            args.bot_root,
            args.web_root,
            args.bot_baseline_sha,
            args.report_dir,
            args.docs_dir,
            web_revision_sha=args.web_revision,
        )
