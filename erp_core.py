...
# --- 3. API CRM, DỰ ÁN, KHO, THU CHI (Giữ nguyên) ---
...
async def add_employee(data: EmployeeReq):
    conn = db_connect(); c = conn.cursor()
    c.execute("INSERT INTO erp_employees (full_name, phone, position, department, base_salary, created_at) VALUES (?, ?, ?, ?, ?, ?)", 
              (data.full_name, data.phone, data.position, data.department, data.base_salary, now_text()))
    conn.commit(); conn.close()
    return {"success": True, "message": "Đã thêm Nhân viên!"}

@router.get("/employees")
async def get_employees():
    conn = db_connect(); c = conn.cursor()
    c.execute("SELECT id, full_name, phone, position, department, base_salary, status FROM erp_employees ORDER BY id DESC")
    data = [{"id": r[0], "full_name": r[1], "phone": r[2], "position": r[3], "department": r[4], "base_salary": r[5], "status": r[6]} for r in c.fetchall()]
    conn.close()
...
async def mark_attendance(data: AttendanceReq):
    ...
    c.execute("INSERT INTO erp_attendance (emp_name, work_date, time_in, status, created_at) VALUES (?, ?, ?, ?, ?)", 
              (data.emp_name, current_date, current_time, data.status, now_text()))
    conn.commit(); conn.close()
    return {"success": True, "message": "Đã chấm công!"}

@router.get("/attendance")
async def get_attendance():
    conn = db_connect(); c = conn.cursor()
    c.execute("SELECT id, emp_name, work_date, time_in, status FROM erp_attendance ORDER BY id DESC LIMIT 50")
    data = [{"id": r[0], "emp_name": r[1], "work_date": r[2], "time_in": r[3], "status": r[4]} for r in c.fetchall()]
    conn.close()
    return {"success": True, "data": data}