...
(function portalShell() {
  ...
  const ALLOWED_STATES = new Set([
    ...
    "completed", "failed", "cancelled", "refunded", "guarded", "disabled", "read_only", "error", "empty"
  ]);
  ...
  function safePayosCheckout(value) {
    ...
    try {
      ...
      return url.protocol === "https:" && (url.hostname === "pay.payos.vn" || url.hostname.endsWith(".payos.vn")) ? url.href : "";
    } catch (_) {
      return "";
    }
  }
  ...
  function renderPaymentEntryPoints(context) {
    ...
    const manualCopy = manualAvailable
      ? "Mở bot đã liên kết, gửi /thucong, chọn mệnh giá/phương thức rồi gửi bill hoặc TXID ngay trong Telegram. Xu chỉ được ghi sau đối soát thật."
  ...
  function renderManualTopupGuide(context) {
    const options = context.paymentOptions && typeof context.paymentOptions === "object" ? context.paymentOptions : {};
    const manual = options.manual && typeof options.manual === "object" ? options.manual : {};
    const available = manual.available === true && Boolean(safeTelegramLink(manual.telegram_url));
    const historySignal = manual.wallet_history_signal_available === true;
    return `<section class="portal-card portal-card-pad" data-manual-topup-guide><div class="portal-card-header"><div><h2 class="portal-card-title">Nạp thủ công: tiếp tục trong Telegram</h2><p class="portal-card-subtitle">Bot canonical giữ toàn bộ state, ảnh bill/TXID, đối soát và quyết định ghi Xu. Web chỉ hướng dẫn và hiển thị dữ liệu ví đã được xác minh.</p></div>${badge(available ? "read_only" : "guarded")}</div>
      <div class="portal-panel-list"><div class="portal-panel-row"><span class="portal-panel-row-icon" aria-hidden="true">1</span><div><strong>Mở bot và gửi <code>/thucong</code></strong><span>Chọn tiền tệ, mệnh giá và phương thức trong cuộc hội thoại Telegram đang được Bot kiểm soát.</span></div></div><div class="portal-panel-row"><span class="portal-panel-row-icon" aria-hidden="true">2</span><div><strong>Gửi chứng từ đúng nơi</strong><span>Ảnh bill hoặc TXID chỉ gửi trong Bot. Không gửi số tài khoản, QR, bill, OTP hay TXID vào Web App.</span></div></div><div class="portal-panel-row"><span class="portal-panel-row-icon" aria-hidden="true">3</span><div><strong>Chờ admin đối soát</strong><span><code>pending_admin_review</code> nghĩa là đang chờ đối soát; chưa phải Xu đã được cộng.</span></div></div><div class="portal-panel-row"><span class="portal-panel-row-icon" aria-hidden="true">4</span><div><strong>Đối chiếu kết quả canonical</strong><span><code>approved</code> mới là đã duyệt; <code>rejected</code> là bị từ chối. Sau khi approved, thay đổi Xu chỉ xuất hiện qua wallet history canonical.</span></div></div></div>
      <div class="portal-form-footer"><span class="portal-form-note">${historySignal ? "Bridge chưa có lịch sử manual-topup đã redaction, vì vậy Web không tra bill/TXID hoặc suy đoán trạng thái; hãy kiểm tra thay đổi Xu sau khi Bot duyệt." : "Lịch sử Xu chỉ xuất hiện khi Core Bridge cấp dữ liệu canonical cho phiên."}</span><a class="portal-button portal-button--quiet" href="/wallet">Xem lịch sử Xu canonical</a></div></section>`;
  }
  ...
  function renderJobDetail(page, context) {
    ...
    return `<article class="portal-page">${renderHero(page, context)}<div class="portal-status-grid">${renderStatusCard(page, context)}${renderSummary(page, context)}</div>
      ...
      <aside class="portal-card portal-card-pad"><div class="portal-card-header"><div><h2 class="portal-card-title">Delivery protection</h2><p class="portal-card-subtitle">Không có download trực tiếp từ path đoán được.</p></div>${job ? assetDeliveryState(job) : deliveryPending()}<div class="portal-notice portal-notice--info" style="margin-top:14px"><span class="portal-notice-icon" aria-hidden="true">i</span><div><strong>Trạng thái hiện tại</strong><p>${safeText(job ? jobStateExplanation(job) : "Chờ Core Bridge kiểm tra ownership trước khi mô tả job.")}</p></div></div>${renderNotes(page)}</aside></div></article>`;
  }
  ...
  function renderAdminDataTable(page, context) {
    ...
    if (["payments", "topups", "revenue", "refunds"].includes(module)) {
      return renderRowsTable(["Mã giao dịch", "Người dùng", "Giá trị", "Xu", "Loại", "Trạng thái", "Cập nhật"], rows, (item) => `<td>${safeText(item.order_code || item.id || "—")}</td><td>${safeText(item.user_id || "—")}</td><td>${safeText(adminNumber(item.amount_vnd, " đ"))}</td><td>${safeText(adminNumber(item.xu, " Xu"))}</td><td>${safeText(item.type || "—")}</td><td>${badge(paymentStatus(item))}</td><td>${safeText(item.paid_at || item.created_at || "—")}</td>`, "Chưa có giao dịch được cấp", "Web App không tính lại số tiền, Xu, refund hoặc trạng thái PayOS.");
    }
    if (["jobs", "failed-jobs", "workers", "runtime"].includes(module)) {
      return renderRowsTable(["Job", "Tính năng", "Trạng thái", "Chi phí canonical", "Cập nhật", "Output engine", "Delivery"], rows, (item) => `<td>${safeText(item.id || "—")}</td><td>${safeText(item.feature || item.job_type || "—")}</td><td>${badge(jobStatus(item))}</td><td>${jobCost(item)}</td><td>${safeText(item.updated_at || item.created_at || "—")}</td><td>${reportedOutput(item)}</td><td>${assetDeliveryState(item)}</td>`, "Chưa có job vận hành được cấp", "Admin view vẫn không hiển thị URL provider, local path hay download không ký.");
    }
